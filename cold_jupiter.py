...
# -*- coding: utf-8 -*-
"""
Created on Tue Dec  2 16:37:50 2025

@author: resulayberksahpaz
Low-mass (>1 AU) planet CPM search pipeline with Gaia DR3 crossmatch
"""
...
# ---------------- PLANET FİLTRESİ ----------------
print("🪐 Düşük kütleli (>1 AU) gezegenler çekiliyor...")

select_fields = [
    "pl_name","hostname","pl_orbsmax","pl_bmassj","ra","dec"
]
if gaia_col:
...
planets = NasaExoplanetArchive.query_criteria(
    ...
    where="pl_bmassj >= 0.3 AND pl_orbsmax >= 1"
).to_pandas()

print(f"➡️ Toplam filtrelenen gezegen sayısı: {len(planets)}")

# ---------- Semimajor axis olmayanlar ----------
missing_smax = planets[planets["pl_orbsmax"].isna()]
missing_smax.to_csv("lowmass_missing_smax.csv", index=False)
print(f"⚠️ Semimajor axis olmayan {len(missing_smax)} sistem 'lowmass_missing_smax.csv' dosyasına kaydedildi.")

# Sadece semimajor axis olanlar pipeline’a girsin
planets = planets.dropna(subset=["pl_orbsmax"])
planets = planets.dropna(subset=["ra","dec"])

print(f"➡️ Pipeline’a girecek gezegen sayısı: {len(planets)}")
print("------------------------------------------------------------")
...
print("🌌 CPM + Parallax companion araması başlıyor...")
print("------------------------------------------------------------")

for idx,row in planets.iterrows():
    hostname = row.get("hostname","")
    pl_name = row.get("pl_name","")
    print(f"\n⭐ {idx+1}/{len(planets)} → {hostname} / {pl_name}")
    ...
    if primary is None:
        ...
        print(f"   🌐 Primary seçildi (en parlak): {int(primary['source_id'])}")
...
output_file = "lowmass_1au_cpm_candidates_gaia_dr3.csv"
df.to_csv(output_file,index=False)

df_no = pd.DataFrame(no_nearby_list)
df_no.to_csv("lowmass_1au_no_nearby.csv", index=False)

print("\n------------------------------------------------------------")
print(f"💾 CPM sonuçları '{output_file}' dosyasına kaydedildi.")
print("💾 Yakın Gaia kaynağı olmayanlar 'lowmass_1au_no_nearby.csv' dosyasına kaydedildi.")
print("💾 Semimajor axis eksik olanlar 'lowmass_missing_smax.csv' dosyasına kaydedildi.")
